...
export const Pph21Content = () => {
  ...
  return (
    <main className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-6">
      <div className="lg:col-span-1 space-y-6 bg-indigo-50 p-6 rounded-xl shadow-md border border-indigo-200">
        ...
        <div className="flex flex-col space-y-1">
          <label className="text-sm font-medium text-gray-700">Status PTKP & Kategori TER</label>
          <select value={ptkpStatus} onChange={(e) => setPtkpStatus(e.target.value)} className="block w-full rounded-lg border-gray-300 py-2.5 px-3 text-base font-spartan focus:border-indigo-600 focus:ring-indigo-600">
            {PTKP_OPTIONS.map(option => (
              <option key={option.value} value={option.value}>{option.label}</option>
            ))}
          </select>
          <p className="text-xs text-gray-500 mt-1">TK: Tidak Kawin, K: Kawin, KI: Kawin Istri Bekerja. Angka adalah tanggungan.</p>
        </div>
      </div>
