    ...
    <main className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
      <div className="space-y-6 bg-slate-50 p-6 rounded-xl border border-slate-200">
        ...
        <div className="flex items-center space-x-2 pt-2">
            <input id="has-npwp" type="checkbox" checked={hasNpwp} onChange={(e) => setHasNpwp(e.target.checked)} className="h-5 w-5 rounded text-indigo-600 border-slate-300 focus:ring-indigo-500" />
            <label htmlFor="has-npwp" className="text-sm font-medium text-slate-700 select-none">Penerima memiliki NPWP</label>
        </div>
        <div className="p-3 bg-white rounded-lg border border-slate-300 text-sm">
            <p className="font-bold text-indigo-700">Tarif PPh Pasal 23 Digunakan:</p>
            <p className="text-slate-900 font-mono tracking-wider text-base">{ratePercentage}% ({hasNpwp ? 'Dengan' : 'Tanpa'} NPWP)</p>
        </div>
      </div>
      <div className="space-y-6">
        ...
        <p className="text-xs text-slate-500 pt-4 px-1">*Catatan: PPh Pasal 23 yang dipotong adalah 2% untuk Jasa (atau 4% tanpa NPWP).</p>
      </div>
    </main>
  );
};